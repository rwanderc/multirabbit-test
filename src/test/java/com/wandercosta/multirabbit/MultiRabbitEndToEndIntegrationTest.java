...
class MultiRabbitEndToEndIntegrationTest {
    ...
    void shouldListenToDefault() {
        final AnnotationConfigApplicationContext ctx = new AnnotationConfigApplicationContext(
                TestConfigs.ThreeBrokersConfig.class, RabbitAutoConfiguration.class,
                MultiRabbitAutoConfiguration.class, ThreeListenersBeans.class);
    ...
    @Test
    @DisplayName("should send to and listen from the broker " + BROKER_NAME_1)
    void shouldListenToBroker1() {
        ...
        await().timeout(Duration.FIVE_SECONDS).untilAsserted(() -> assertEquals(message, MEMORY.get(BROKER_NAME_1)));

        ctx.close(); // Close and stop the listeners
    }

    @Test
    @DisplayName("should send to and listen from the broker " + BROKER_NAME_2)
    void shouldListenToBroker2() {
        final AnnotationConfigApplicationContext ctx = new AnnotationConfigApplicationContext(
                TestConfigs.ThreeBrokersConfig.class, RabbitAutoConfiguration.class,
                MultiRabbitAutoConfiguration.class, ThreeListenersBeans.class);
        ...
        await().timeout(Duration.FIVE_SECONDS).untilAsserted(() -> assertEquals(message, MEMORY.get(BROKER_NAME_2)));
